export function RiskBadge({ score }: { score: number | null }) {
  ...
  const color =
    score <= 3 ? 'bg-green-900 text-green-300' :
    score <= 5 ? 'bg-yellow-900 text-yellow-300' :
    score <= 7 ? 'bg-orange-900 text-orange-300' :
...
export function RecBadge({ rec }: { rec: string | null }) {
  ...
  const map: Record<string, string> = {
    strong_buy: 'bg-green-900 text-green-200',
    buy: 'bg-green-800 text-green-300',
    negotiate: 'bg-yellow-900 text-yellow-300',
    caution: 'bg-orange-900 text-orange-300',
  ...
  return (
    <span className={`${map[rec] ?? 'bg-gray-800 text-gray-400'} text-xs px-2 py-0.5 rounded-full font-medium`}>
      {labels[rec] ?? rec}
    </span>
  );
}
...
export function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`${map[status] ?? 'bg-gray-800 text-gray-300'} text-xs px-2 py-0.5 rounded-full`}>
      {status}
    </span>
  );
}
